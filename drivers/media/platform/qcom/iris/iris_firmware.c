...
static int iris_load_fw_to_memory(struct iris_core *core)
{
	...
	if (!core->pas_ctx) {
		core->pas_ctx = devm_qcom_pas_context_alloc(core->dev, IRIS_PAS_ID,
							    mem_phys, res_size);
		if (IS_ERR(core->pas_ctx))
			return PTR_ERR(core->pas_ctx);
	}

	firmware = iris_detect_firmware(core, fw_dev, &fw_name);
	if (IS_ERR(firmware))
...
err_pas_shutdown:
	qcom_pas_shutdown(core->pas_ctx->pas_id);
err_unmap:
...
int iris_fw_unload(struct iris_core *core)
{
	...
	ret = qcom_pas_shutdown(core->pas_ctx->pas_id);
	iris_fw_iommu_unmap(core);
