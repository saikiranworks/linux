...
static int qcom_pas_shutdown_any(int pas_id, int lite_pas_id)
{
	 ...
	 */
	if (lite_pas_id)
		lite_ret = qcom_pas_shutdown(lite_pas_id);
...
static int qcom_pas_stop(struct rproc *rproc)
{
	...
	ret = qcom_pas_shutdown_any(pas->pas_id, pas->lite_pas_id);
	...
	if (pas->dtb_pas_id) {
		ret = qcom_pas_shutdown_any(pas->dtb_pas_id, pas->lite_dtb_pas_id);
		if (ret)
